...
const ProfilPorteurProjet = (props) => {
  ...
  const payer = async () => {
    ...
    try {
      const montant =
        supPay <= 0
          ? moneyFormat(selectedPlage?.frais_abonnement)
          : moneyFormat(supPay);
  ...
  React.useEffect(() => {
    ...
    function loadPlage() {
      PlageInvestissementService.getAll().then((rs) => {
            ...
            <div className="tab-content">
              ...
              >
                <div className="container my-5">
                  ...
                  <p className="text-muted mb-5">
                    Modifier votre plage d'investissement.
                  </p>
                        ...
                        >
                          ...
                          <div className="card-body">
                            <h1 className="card-title pricing-card-title">
                              {moneyFormat(item.frais_abonnement)} FCFA
                            </h1>
                            <p className="mt-2">
                              Possibilité d'investir sur des projets avec un
                              besoin chiffre d'affaire prévissionnel compris
                              entreprise&nbsp;
                              <strong>
                                {!item.montant_min || item.montant_min === 0
                                  ? item.min
                                  : moneyFormat(item.montant_min) + " FCFA"}
                              </strong>{" "}
                              et&nbsp;
                              <strong>
                                {!item.montant_max || item.montant_max === 0
                                  ? item.max
                                  : moneyFormat(item.montant_max) + " FCFA"}
                              </strong>
                            </p>
                            {item.id === user?.profil && (
                              <Button
      ...
      >
        ...
        <Modal.Body>
          <p>
            ...
            pour changer votre profil en <strong>{selectedPlage?.type}</strong>.{" "}
          </p>
          <hr />
