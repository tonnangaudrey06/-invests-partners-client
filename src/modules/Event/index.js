    ...
    return (
        <Container header active="events" footer>
            ...
            <div className="container my-5">
                ...
                {(months || []).filter(item => moment(item.date_debut).month() === currentMonth && moment(item.date_debut).year() === currentYear).map((item) => (
                ...
                {(events || []).filter(item => moment(item.date_debut).month() !== currentMonth || moment(item.date_debut).year() !== currentYear).map((item) => (
                    <CardEvent item={item} t={t} key={item.id}/>
                ))}
            </div>
            </div>
